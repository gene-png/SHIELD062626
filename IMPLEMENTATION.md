...
> **Remaining (multi-increment):** the `csf_score` job (suggests dims + narrative only) wired as a Run-AI; IG metric + ZT/ATT&CK cross-ref reference data (to drive roll-up Rules 2/5 + priority `is_core`); the 10 Playbook sub-pages; the exact XLSX shapes + 20-30pp report.

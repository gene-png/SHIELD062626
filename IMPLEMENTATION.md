...
> **Remaining (multi-increment):** replace the simplified `csf_answers.maturity_tier` with tiered Working Profiles (HIGH/MOD/LOW × ~108 subcats × 5 dimension scores + in-scope/rationale/narrative/evidence/per-subcat target), the Enterprise Profile, IG metric + ZT/ATT&CK cross-ref reference data, the `csf_score` job (suggests dims + narrative only), the 10 Playbook sub-pages, and the exact XLSX shapes + 20-30pp report.
...
> **Status `[~]`** — deterministic tier engine done: `app/risk/engine.py` (`tier_for` NIST 800-30 5x5 incl. the special Critical cases, `risk_score`, `cadence_for`, `tier_counts`/`axis_counts`/`action_counts`/`matrix_counts`) + Likelihood/Impact/RiskTier/RiskAxis/RecommendedAction enums — pure + unit-tested (`test_risk_engine.py`). **Remaining:** generation gate, versioned register model + migration, `risk_synthesize` job (links validated against catalogs), provenance, dashboard + XLSX/PDF/Word exports.
...
| 2026-06-25 | E engine | Risk Register 5x5 tier engine `app/risk/engine.py` (`tier_for` w/ the special Critical cases, score, cadence, matrix/tier/axis/action rollups) + enums — pure + unit-tested (`test_risk_engine.py`). Register model, `risk_synthesize`, gate, exports remain. |

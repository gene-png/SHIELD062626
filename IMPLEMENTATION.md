...
| 2026-06-25 | E exports | `app/risk/exporters.py` (XLSX all-fields + blank governance columns; PDF/Word = KPI + 5x5 matrix + tier legend + table); `POST …/register/export` renders + stores all 3 as artifacts (migration 0023). Export-download test green. E backend now complete; only the dashboard FE remains. |

"""Draft-exists guard on POST /zt/services/{id}/assessments (SPRINT_3 T1).

Ported from the CSF T7 guard (tests/unit/test_csf_draft_guard.py). The ZT mint
route historically cut a fresh draft version on EVERY call and pre-seeded ~87
capability rows per mint, so a client hammering "start assessment" produced
unbounded v2, v3, v4… drafts. The guard now returns the open draft idempotently
(HTTP 200) instead of minting, and only cuts a new version once the prior draft
has moved on (approved).
"""
...
def app_client(tmp_path) -> Iterator[TestClient]:
    db_path = tmp_path / "shield-zt-draft-guard.db"
...
def _open_service(c: TestClient, bearer: str) -> str:
    r = c.post(
        ...
        json={"kind": "zero_trust_cisa", "title": "Zero Trust (CISA)"},
    )
    assert r.status_code == 201, r.text
    return r.json()["id"]
...
def test_new_version_minted_after_prior_draft_closed(app_client) -> None:
    ...
    approved = c.post(
        f"/zt/assessments/{assessment_id}/approve",
        headers={"Authorization": f"Bearer {bearer}"},
    )

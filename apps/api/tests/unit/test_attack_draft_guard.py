"""Draft-exists guard on POST /attack/services/{id}/assessments (SPRINT_3 T1).

Ported from the CSF T7 guard (tests/unit/test_csf_draft_guard.py). The attack
mint route historically cut a fresh draft version on EVERY call and pre-seeded
~600 coverage rows per mint, so a client hammering "start assessment" produced
unbounded v2, v3, v4… drafts (the worst offender of the shared pattern). The
guard now returns the open draft idempotently (HTTP 200) instead of minting,
and only cuts a new version once the prior draft has moved on (approved).
"""
...
def app_client(tmp_path) -> Iterator[TestClient]:
    db_path = tmp_path / "shield-attack-draft-guard.db"
...
def test_new_version_minted_after_prior_draft_closed(app_client) -> None:
    ...
    approved = c.post(
        f"/attack/assessments/{assessment_id}/approve",
        headers={"Authorization": f"Bearer {bearer}"},
    )
